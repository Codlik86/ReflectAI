...
async def list_payments(
    ...
    stmt = select(Payment).order_by(desc(Payment.id)).limit(limit).offset(offset)
...
async def list_subscriptions(
    ...
    stmt = select(Subscription).order_by(desc(Subscription.user_id)).limit(limit).offset(offset)
    rows = (await session.execute(stmt)).scalars().all()
...
# триал
from app.billing.service import start_trial_for_user  # type: ignore
