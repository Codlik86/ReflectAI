...
// === TOUCH-ХЕЛПЕРЫ: убираем «двойной тап» на мобильных ===
...
export default function Breath478() {
  ...
  const startPhase = (next: Phase) => {
    ...
    const durSec = next === "inhale" ? INHALE_SEC : next === "hold" ? HOLD_SEC : EXHALE_SEC;
    const durMs = durSec * 1000;
    currentDurMs.current = durMs;
    endAt.current = performance.now() + durMs;

    tick(durMs, () => {
      // последовательность: inhale → hold → exhale → (след. цикл)
  ...
  const goStart = () => {
    trackEvent("miniapp_action", "exercise_started", { exercise_id: "breath-478" });
    resetAll();
  };
  ...
  const circleColor = phase === "inhale" ? "#3B82F6" : phase === "exhale" ? "#22C55E" : "#F59E0B";

  return (
    <div className="min-h-dvh flex flex-col">
      <BackBar title="Дыхание 4–7–8" to="/exercises" />
      ...
      <div className="px-5 space-y-4" style={{ paddingBottom: "calc(env(safe-area-inset-bottom) + 98px)" }}>
        {phase === "intro" && (
          <div className="rounded-3xl bg-white p-5 space-y-4">
            <div className="text-[18px] font-medium text-ink-900">Переходим в спокойный режим</div>
            <div className="text-[16px] leading-7 text-ink-700">
              Вдох 4 сек — пауза 7 сек — выдох 8 сек. Всего 8 циклов.
            </div>
        ...
        {(phase === "idle" || phase === "inhale" || phase === "hold" || phase === "exhale") && (
          <div className="rounded-3xl bg-white p-5">
            ...
            <div className="mt-3 text-center text-[14px] text-ink-600">
              {phase === "idle"   && "Готов к старту: вдох 4 сек → пауза 7 сек → выдох 8 сек."}
              {phase === "inhale" && "Вдыхай носом, расширяя живот."}
              {phase === "hold"   && "Пауза — мягкая задержка без напряжения."}
              {phase === "exhale" && "Выдыхай длиннее, чем вдох."}
            </div>
